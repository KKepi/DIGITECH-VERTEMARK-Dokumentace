...
│   │   ├── TreeStructure.cs              # (Neanalyzováno - pravděpodobně nepoužito)
│   │   │
...
| `fo-dicom.Drawing` | 4.0.8 | Renderování DICOM obrázků |
...
```
Vrací singleton instanci Project.

```csharp
public void LoginNewUser(string id, bool validator)
```
**Parametry:**
- `id`: Identifikátor uživatele (řetězec)
- `validator`: True, pokud je uživatel validátor, false pokud anotátor

**Popis:** Vytvoří a uloží novou instanci User pro aktuální relaci.
...
**Parametry:**
- `savingParameter`: 
  - `0`: Uložit do složky `to_anotate`
  - `1`: Uložit do složky `to_validate`
  - `2`: Uložit do složky `validated`
  - `3`: Uložit do složky `invalid`
- `button`: Název tlačítka, které spustilo uložení (pro metadata)

**Popis:** Uloží aktuální projekt se všemi anotacemi, metadaty a obrázky do příslušné složky na základě stavu workflow.
...
```
**Popis:** Získá nebo nastaví aktuální stav aplikace. Nastavení této vlastnosti spustí událost `StateChanged`.

**Události:**
- `StateChanged`: Spuštěna při změně stavu, předává novou hodnotu `AppState`.
...
```
**Vrací:** Slovník mapující ID anotace na seznam souřadnicových n-tic (x, y).

**Popis:** Serializuje data anotace do formátu slovníku. Pro anotace Vertebra vrací souřadnice bodů. Pro Implant/Fusion vrací souřadnice pixelů z bitmapy canvasu.
...
```
**Popis:** Extrahuje PNG obrázek z DICOM souboru specifikovaného v `dicomPath`. Nastaví vlastnosti `pngPath` a `jsonPath`.

**Vyvolává:** Může vyvolat výjimky, pokud je DICOM soubor neplatný nebo nedostupný.
...
1. **Líné renderování**: Renderovat bitmapu canvasu pouze při ukládání, ne při každém tahu
2. **Inkrementální aktualizace ZIP**: Sledovat změněné soubory a aktualizovat pouze tyto záznamy
3. **Cache pixelů**: Ukládat seznamy anotovaných pixelů do mezipaměti místo přepočítávání
4. **Virtualizace**: Pro projekty s mnoha anotacemi implementovat virtualizaci UI
5. **Asynchronní operace**: Přesunout I/O operace na pozadí, aby se zabránilo blokování UI
